...
const { capabilities: BUILTIN_CAPABILITIES, combined: BUILTIN_PRESET } = resolveCapabilities(RAW_CAPABILITIES);
...
app.get('/health', (_req, res) => {
  ...
  res.json({
    ...
    runners: { total: runners.size, online: online.length, offline: runners.size - online.length },
  });
});
...
app.get('/metrics', (_req, res) => {
  ...
  for (const [id, runner] of runners) {
    if (runner.resources && runner.status === 'online') {
      ...
      lines.push(`yggdrasil_runner_cpu_percent{${labels}} ${runner.resources.cpu.percent}`);
      ...
      lines.push(`yggdrasil_runner_memory_percent{${labels}} ${runner.resources.memory.percent}`);
    }
  }

  // Expected runner version info
  if (EXPECTED_RUNNER_VERSION) {
    lines.push('# HELP yggdrasil_expected_runner_version Expected runner version (always 1) — label carries expected version');
    lines.push('# TYPE yggdrasil_expected_runner_version info');
    lines.push(`yggdrasil_expected_runner_version{version="${EXPECTED_RUNNER_VERSION}"} 1`);
  }

  // Per-runner version info
  for (const [id, runner] of runners) {
    const verLabels = `runner="${id}",name="${runner.name}",version="${runner.version}"`;
    lines.push(`# HELP yggdrasil_runner_version_info Runner version (always 1) — labels carry version`);
    ...
    if (EXPECTED_RUNNER_VERSION && runner.version !== EXPECTED_RUNNER_VERSION) {
      ...
      lines.push(`# HELP yggdrasil_runner_outdated Outdated runner flag (1 = version mismatch)`);
    ...
    if (runner.pendingUpdate) {
      ...
      lines.push(`# HELP yggdrasil_runner_pending_update Pending update flag (1 = update pending)`);
...
app.post('/runners/register', (req, res) => {
  ...
  console.log(`[yggdrasil] runner registered: ${runnerId} (${body.name || 'unknown'})`);
...
app.post('/runners/:runnerId/request-update', (req, res) => {
  ...
  console.log(`[yggdrasil] update requested for runner ${req.params.runnerId}: version=${version}`);
  res.json({
...
app.post('/runners/:runnerId/tasks', (req, res) => {
  ...
  console.log(`[yggdrasil] task created: ${task.taskId} on runner ${req.params.runnerId} (type=${task.type || '?'})`);
...
setInterval(() => {
  ...
  for (const [runnerId, runner] of runners) {
    ...
    if (elapsed > LEASE_TTL_MS) {
      runner.status = 'offline';
      console.warn(`[yggdrasil] runner ${runnerId} marked offline (no heartbeat for ${Math.round(elapsed / 1000)}s)`);
    }
  }
}, 10_000);
...
const llmBaseUrl = process.env.LLM_BASE_URL || 'http://host.docker.internal:1234/v1';
const llmApiKey = process.env.LLM_API_KEY || process.env.OASIS_OPENAI_API_KEY || '';
const maxToolIterations = parseInt(process.env.AGENT_MAX_TOOL_ITERATIONS || process.env.LLM_MAX_TOOL_ITERATIONS || '25', 10);
...
function registerBuiltinRunner(): void {
  ...
  console.log(`[yggdrasil] activated presets: ${RAW_CAPABILITIES.join(', ')} → capabilities: ${BUILTIN_CAPABILITIES.join(', ')}`);
}
  ...
  try {
    switch (name) {
      case 'shell': {
        ...
        const result = execSync(args[0], { timeout: 30_000, encoding: 'utf-8', stdio: 'pipe', shell: '/bin/sh' });
      ...
      case 'write_file': {
        ...
        return { success: true, output: `Written ${args[1].length} bytes to ${args[0]}` };
      }
      case 'web_search': {
      ...
      case 'web_fetch': {
        const { default: axios } = await import('axios');
        const res = await axios.get(args[0], { timeout: 30_000, responseType: 'text' });
        return { success: true, output: (typeof res.data === 'string' ? res.data : JSON.stringify(res.data)).slice(0, 100_000), error: '' };
      }
      case 'python': {
        const { execSync } = await import('child_process');
        const pyResult = execSync(`/app/python-venv/bin/python3 -c ${JSON.stringify(args[0])}`, {
          timeout: 60_000, encoding: 'utf-8', stdio: 'pipe', maxBuffer: 10 * 1024 * 1024,
        });
        return { success: true, output: pyResult.trim().slice(0, 100_000), error: '' };
      }
      case 'node': {
        const { execSync } = await import('child_process');
        ...
        return { success: true, output: nodeResult.trim().slice(0, 100_000), error: '' };
      }
      case 'github': {
        const { execSync } = await import('child_process');
        ...
        return { success: true, output: ghResult.trim().slice(0, 100_000), error: '' };
      }
      default:
...
async function executeTask(taskId: string, goal: string): Promise<void> {
  ...
  executingTasks.set(taskId, { taskId, goal, startedAt: Date.now(), abortController });

  try {
    console.log(`[yggdrasil] executing built-in task ${taskId}: ${goal.slice(0, 80)}`);
  ...
  for (let iter = 0; iter < maxToolIterations; iter++) {
    ...
    if (toolMatch) {
      ...
      try {
        ...
        messages.push({ role: 'user', content: 'Invalid JSON in tool block. Use valid JSON.' });
        continue;
      }

      const result = await executeTool(call.name, Object.values(call.arguments));
      const observation = result.success
        ? `Output:\n${result.output}`
        : `Error: ${result.error || result.output}`;

      console.log(`[yggdrasil] tool ${call.name} iter=${iter} success=${result.success}`);
    ...
    messages.push({ role: 'assistant', content });
    messages.push({ role: 'user', content: 'Respond with a `tool` block or a `final` block.' });
  }

  if (!finalAnswer) finalAnswer = 'Max iterations reached without a final answer.';
...
function pollBuiltinTasks(): void {
  ...
  for (const task of running) {
    ...
    console.log(`[yggdrasil] built-in runner picked up task ${task.taskId}: ${goal.slice(0, 80)}`);
...
function heartbeatBuiltinRunner(): void {
  ...
  runner.tasks = runner.tasks.map((t) => {
    ...
    if (executing) {
      return { ...t, startedAt: executing.startedAt, metadata: { ...t.metadata, goal: executing.goal.slice(0, 120) } };
    }
    return t;
  });
}
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`[yggdrasil] built-in runner active (maxConcurrent=${MAX_CONCURRENT_TASKS}, llmConfigured=${!!llmApiKey})`);
});
