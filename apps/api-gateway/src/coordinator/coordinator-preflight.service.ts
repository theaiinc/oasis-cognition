/**
 * CoordinatorPreflightService — computes host capacity + Yggdrasil admission
 * + cost estimate for a planner plan, then decides whether user approval is
...
export class CoordinatorPreflightService {
  private readonly logger = new Logger(CoordinatorPreflightService.name);
  ...
  /** Run full preflight for a plan. Returns the preflight result with
   *  admission decision and proposed budget. Does NOT mutate any state
   *  — that happens in CoordinatorService.createJob + approveJob. */
  async preflight(
    plan: PlannerPlan,
    ...
    for (const t of planTasks) {
      const model = t.profile_id || null; // simplified — real impl would resolve profile
    ...
    // If no tasks from planner, t-shirt size from step count
    ...
    const gpuLimit = host.gpu_vram_mb !== null ? GPU_SLOTS : 999; // if no GPU, not constrained here
    ...
    if (parallelCount < 2) {
      // Nothing to parallelise — not a degradation
