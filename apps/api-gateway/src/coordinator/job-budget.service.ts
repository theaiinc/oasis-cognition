...
import { JOB_BUDGET_KEY } from './coordinator.types';
import type { JobBudget } from './coordinator.types';
...
export class JobBudgetService {
  private readonly logger = new Logger(JobBudgetService.name);
  ...
  constructor() {
    ...
    try {
      ...
      this.redis.connect()
        ...
        .catch(() => this.logger.warn('Redis unavailable; job budget held in memory only'));
    } catch {
      this.logger.warn('Redis init failed; job budget held in memory only');
    }
  }
  ...
  async resetBudget(jobId: string): Promise<void> {
    ...
    if (this.redis && this.redisReady) {
      ...
      catch (err) { this.logger.warn(`resetBudget redis: ${err}`); }
    }
  }
}
